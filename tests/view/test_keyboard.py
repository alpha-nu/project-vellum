from view.keyboard import read_keyboard_key, KeyboardKey


def test_read_keyboard_key_arrow_up(monkeypatch):
    seq = iter(["\x1b", "[", "A"])
...
def test_read_keyboard_key_char(monkeypatch):
    monkeypatch.setattr("view.keyboard.readchar.readchar", lambda: "x")
    token = read_keyboard_key()
    assert token.key == KeyboardKey.CHAR and token.char == "x"


def test_read_keyboard_key_mappings_all(monkeypatch):
    """read_keyboard_key should return KeyboardToken for various inputs"""
    # Arrow up
    seq = iter(["\x1b", "[", "A"])
    monkeypatch.setattr("view.keyboard.readchar.readchar", lambda: next(seq))
    token = read_keyboard_key()
...
def test_read_keyboard_key_unknown_escapes(monkeypatch):
